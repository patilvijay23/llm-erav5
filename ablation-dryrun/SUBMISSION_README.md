...
> **Status:** testable design hypothesis, not a claimed training result. The reportable 1B and 3B proxy runs have **not** been executed. A CPU-only 8GB laptop profile is included only to validate P7 data preparation, deterministic mixture accounting and repository plumbing; it is not presented as evidence about 1B-model stability.
...
- P1 is preferred over P7 when the matched linear-transition control either passes the transition gates that P7 fails, or reduces the gradient-norm or loss spike multiplier by at least **10%**;
...
## P7 implementation — hard shift versus matched linear control

The runnable P7 package is included under `p7-ablation-fineweb-indiccorp/`.
The **reportable experiment requires two matched 1B/20B-token runs** using the
same tokenizer, model initialization, manifests, per-source permutations,
optimizer and learning-rate schedule:

1. **P7 hard arm:** 50:50 FineWeb-Edu/IndicCorpV2 until the 10B-token boundary,
   followed by an immediate 80:20 shift.
2. **Warm control:** the same 50:50 → 80:20 change linearly blended across a
   centred 256M-token window.

The centred warm window preserves the same complete-run source totals as P7:

| Arm accounting | FineWeb-Edu | IndicCorpV2 | Total |
|---|---:|---:|---:|
| Before the 10B centre | 5B in the hard arm | 5B in the hard arm | 10B |
| Complete run, hard or linear | **13B** | **7B** | **20B** |

### Correct pool and holdout accounting

Each manifest contains **10B trainable tokens**, and the evaluation holdout is
prepared in addition to that pool:

| Per source | Tokens |
|---|---:|
| Trainable candidate pool | 10,000,000,000 |
| Evaluation holdout: 10,000 × 640 | 6,400,000 |
| Total packed | 10,006,400,000 |

This means FineWeb replay is exactly **3B tokens**, not 3.0064B. Seven billion
IndicCorp training tokens are selected and 3B remain unused.

### Decision metrics

Both arms report:

- training and fixed-holdout loss;
- pre-clipping gradient norm;
- exact source share and source-pass count;
- transition start, centre and end; and
- recovery time after the transition.

A run passes the initial transition gate only when:

- peak gradient norm is at most **2.5×** the preceding median;
- peak loss is at most **1.15×** the preceding median; and
- both recover inside the gates within **500 optimizer steps**.

The warm control is preferred when the arms are configuration-matched and it
passes gates that the hard arm fails, or reduces either spike multiplier by at
least **10%**. Raw values and seed spread must still be reported; the 10%
materiality threshold is itself a hypothesis.

This ablation tests only a **50:50 → 80:20** shift that downweights IndicCorpV2.
It is not evidence for the reverse direction or every transition in the full
ten-lane curriculum.

### Laptop scope

The available CPU-only 8GB laptop cannot train the fixed ~996M-parameter model.
Its profile prepares **1.28M trainable tokens plus 12.8K held-out tokens per
source** and validates the exact scaled schedule, manifests, deterministic
FineWeb replay, memory-mapped shards and unit tests. It is implementation
evidence only and is not reported as P7 stability evidence.

The full 1B hard and linear runs remain marked **not executed** until suitable
multi-GPU compute is available. Exact commands and the comparison workflow are
in the P7 `README.md`.
