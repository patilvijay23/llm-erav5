#!/usr/bin/env python3
"""Reproducible Session 4 cleaner for AI4Bharat Samanantar.

The cleaner is bilingual-pair aware. It normalizes Indic text without deleting
ZWJ/ZWNJ, validates source/target structure, applies conservative quality rules,
checks scripts, masks structured PII, deduplicates normalized pairs globally,
checks supplied held-out fingerprints, and emits a deterministic manifest.

It deliberately does not pretend that heuristics prove translation equivalence.
Use the Samanantar v0.3 LaBSE score metadata (or another audited multilingual
similarity score) via --semantic-score-field for semantic alignment filtering.
"""
...
def iter_hf(dataset_id: str, languages: Iterable[str]) -> Iterator[dict[str, Any]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    rows = iter_hf(args.hf_dataset, languages) if args.hf_dataset else iter_local(args.input)
    ...
    seen_pairs: set[str] = set(); source_targets: dict[tuple[str,str], str] = {}; output_hashes: list[str] = []
    clean_path=args.output_dir/"cleaned.jsonl"; reject_path=args.output_dir/"rejected.jsonl"
    print("args loaded...")
    with clean_path.open("w",encoding="utf-8") as cf, reject_path.open("w",encoding="utf-8") as rf:
        print("file opened...")
        # print(f"loop length will be {len(rows)}")
        for i,row in enumerate(rows):
            if i%100000 == 0: print(f"\tloop {i}")
            if args.limit is not None and i >= args.limit: break
            counts["input_rows"] += 1
            lang=choose_language(row,args.language); langs[lang]+=1
            d,n,pstats=evaluate_pair(row,lang,args,heldout); norm_total.update(n); pii_total.update(pstats)
            if d["keep"] and d["pair_sha256"] in seen_pairs:
                d["keep"]=False; d["reasons"].append("exact_pair_duplicate")
            source_key=(lang,normalized_key(d["src"])); target_key=normalized_key(d["tgt"])
            if source_key in source_targets and source_targets[source_key] != target_key:
                d["flags"].append("source_has_multiple_targets")
            else: source_targets[source_key]=target_key
            for f in set(d["flags"]): flags[f]+=1
            payload={"idx":row.get("idx",i),"src":d["src"],"tgt":d["tgt"],"language":lang,
                     "data_source":row.get("data_source"),"cleaning":{"pair_sha256":d["pair_sha256"],
                     "flags":sorted(set(d["flags"])),"script_ratios":d["script_ratios"],"semantic_score":d["semantic_score"]}}
            if d["keep"]:
                seen_pairs.add(d["pair_sha256"]); output_hashes.append(d["pair_sha256"]); counts["cleaned_rows"]+=1
                cf.write(canonical_json(payload)+"\n")
            else:
                counts["rejected_rows"]+=1
                for r in set(d["reasons"]): reasons[r]+=1
                rf.write(canonical_json({"row":payload,"reasons":sorted(set(d["reasons"]))})+"\n")
    print("loop and file done...")
    script_hash=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    manifest={
      "schema_version":2,
      "dataset":{"id":args.hf_dataset or str(args.input),"languages":languages if args.hf_dataset else [args.language or "from_rows"],
                 "license_note":args.license_note,"src_field":args.src_field,"tgt_field":args.tgt_field},
      "cleaner":{"script_sha256":script_hash,"unicode_form":"NFC","preserved_codepoints":["U+200C","U+200D"],
                 "semantic_filter":f"{args.semantic_score_field} >= {args.semantic_threshold}" if args.semantic_score_field else "not executed: no score field",
